    ...
    const fetchNotifications = async () => {
      try {
        ...
        for (const request of requests) {
          ...
          if (employee?.employeeType!=="ADMIN" &&  ( (request.secondApprovment === 'APPROVED' || (request.secondApprovment === 'REJECTED' && request.firstApprovment === 'APPROVED' )  ) && (employee?.employeeId===employeeId))) {
            console.log("+++++++++++++++++++::p ", employee?.employeeType);
            const { sortTime, time } = resolveTime(request.approvalDate2 || request.updatedAt);
            items.push({
              ...request,
    ...
    <header className="sticky top-0 z-50 flex h-16 shrink-0 items-center justify-between border-b bg-white dark:bg-gray-900 px-4 lg:px-6 shadow-sm">
      ...
      <div className="flex items-center">
        ...
        <div className="relative mr-4" ref={dropdownRef}>
          ...
          {showNotifications && (
            <div className="fixed sm:absolute inset-x-0 sm:inset-x-auto top-16 sm:top-10 sm:left-0 sm:mt-2 w-full sm:w-96 max-h-[calc(100vh-5rem)] sm:max-h-96 rounded-none sm:rounded-lg shadow-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 z-50 overflow-hidden">
              {/* Header */}
              <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 flex justify-between items-center">
                <h3 className="font-semibold text-gray-800 dark:text-white text-lg sm:text-base">الإشعارات</h3>
                {unreadCount > 0 ? (
                  <button
                    onClick={markAllAsRead}
                    className="text-sm text-blue-600 dark:text-blue-400 hover:underline px-3 py-1 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
                    aria-label="Mark all notifications as read"
                  >
                    تعيين الكل كمقروء
                  </button>
              ...
              <div className="max-h-[calc(100vh-10rem)] sm:max-h-80 overflow-y-auto">
                {notifications.length > 0 ? (
                  notifications.map((notification) => (
                    <div
                      key={notification.id}
                      className={`px-4 py-4 sm:py-3 border-b border-gray-100 dark:border-gray-700 last:border-b-0 transition-colors cursor-pointer group ${
                        !notification.read 
                          ? 'bg-blue-50 dark:bg-blue-900/20 border-r-2 border-r-blue-500' 
                          : 'hover:bg-gray-50 dark:hover:bg-gray-700'
                      }`}
                    ...
                    >
                      <div className="flex items-start space-x-3 space-x-reverse">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm sm:text-xs text-gray-500 dark:text-gray-400 mt-2 sm:mt-1">
                            {notification.time}
                          </p>
                        </div>
                        {!notification.read && (
                          <div className="w-3 h-3 sm:w-2 sm:h-2 rounded-full bg-blue-500 flex-shrink-0 mt-2 sm:mt-2"></div>
                        )}
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="px-4 py-12 sm:py-8 text-center text-gray-500 dark:text-gray-400">
                    <svg className="w-16 h-16 sm:w-12 sm:h-12 mx-auto text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                    </svg>
                    <p className="mt-4 sm:mt-2 text-lg sm:text-base">لا توجد إشعارات حالياً</p>
                    <p className="text-sm sm:text-xs mt-2 text-gray-400 dark:text-gray-500">
              ...
              {/* {notifications.length > 0 && (
                <div className="p-4 sm:p-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                  <button 
                    className="w-full text-center text-base sm:text-sm text-blue-600 dark:text-blue-400 hover:underline py-2 sm:py-1 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
                    onClick={() => {
